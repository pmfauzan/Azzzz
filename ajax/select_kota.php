...
if (!empty($_GET['q'])){
	if (ctype_digit($_GET['q'])) {
		include '../connect.php';
		$link=connect_db();
		$sql="SELECT * FROM inf_lokasi where lokasi_propinsi=$_GET[q] and lokasi_kecamatan=0 and lokasi_kelurahan=0 and lokasi_kabupatenkota!=0 order by lokasi_nama";
...
if (empty($_GET['kel'])){
	...
	if (!empty($_GET['kec']) and !empty($_GET['prop'])){
		...
		include '../connect.php';
		$link=connect_db();
		$sql="SELECT * FROM inf_lokasi where lokasi_propinsi=$_GET[prop] and lokasi_kecamatan!=0 and lokasi_kelurahan=0 and lokasi_kabupatenkota=$_GET[kec] order by lokasi_nama";
			$query = mysqli_query($link,$sql);
	...
	if (!empty($_GET['kec']) and !empty($_GET['prop'])){
		...
		include '../connect.php';
		$link=connect_db();
		$sql="SELECT * FROM inf_lokasi where lokasi_propinsi=$_GET[prop] and lokasi_kecamatan=$_GET[kel] and lokasi_kelurahan!=0 and lokasi_kabupatenkota=$_GET[kec] order by lokasi_nama";
			$query = mysqli_query($link,$sql);
